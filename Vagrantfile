...
jsonContent = File.read("#{dir}/puphpet/config.json")

configValues = JSON.parse(jsonContent)
...
Vagrant.configure("2") do |config|
  ...
  data['vm']['synced_folder'].each do |i, folder|
    if folder['source'] != '' && folder['target'] != '' && folder['id'] != ''
      type = folder['type']
      config.vm.synced_folder "#{folder['source']}", "#{folder['target']}", id: "#{folder['id']}", type: type, synced_folder_type: type, disabled: folder['disabled']
    end
  end
  ...
  config.vm.provision :puppet do |puppet|
    ...
    ssh_superuser = !configValues['ssh_auth_keys']['ssh_superuser'].nil? ? configValues['ssh_auth_keys']['ssh_superuser'] : "root"
    puppet.facter = {
      "ssh_username"  => "#{ssh_username}",
      "ssh_superuser" => "#{ssh_superuser}"
    }
